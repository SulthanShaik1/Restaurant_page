...
function AppInner() {
  ...
  const filtersApplied = useMemo(() => {
    ...
    const cuisinesArray = Array.isArray(filters.cuisines)
      ? filters.cuisines
      : typeof filters.cuisine === "string" && filters.cuisine !== "All"
        ? (filters.cuisine as string).split(",").map((s: string) => s.trim()).filter(Boolean)
        : [];
    const hasCuisines = cuisinesArray.length > 0;
  ...
  return (
    <>
      <header className="header d-flex align-items-center justify-content-between px-3">
        {/* BRAND: stack on small screens, align row on md+ */}
        ...
        {/* RIGHT SIDE OF HEADER - allow wrapping on small screens */}
        <div className="d-flex align-items-center gap-3 flex-wrap">
          <button
            className="btn btn-outline-secondary d-flex align-items-center filters-btn gap-6"
      ...
      {/* HERO: show ONLY when no filters are applied */}
      {!filtersApplied && <div className="container mt-4"><Hero /></div>}
